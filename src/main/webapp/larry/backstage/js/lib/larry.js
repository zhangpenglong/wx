/**
 * @author lishuang 2017-08-30 /参考larrycms并进行改进
 * @link      http://www.dongao.com/
 * @version   [0.01]
 * @license   [license]
...
layui.define(['jquery','layer','element','common'],function(exports){
     ...
     Navbar.prototype.render = function(){
     	...
     	if(typeof(_config.elem) !== 'string' && typeof(_config.elem) !== 'object'){
     		common.cmsError('ERROR: elem参数未定义或设置出错','elem参数出错');
     	}
     	// 若为字符串
     	var $container;
     	...
     	if ($container.length === 0) {
            common.cmsError('ERROR:找不到elem参数配置的容器，请检查.', '容器参数配置出错');
        }
        if (_config.data === undefined && _config.url === undefined) {
            common.cmsError('ERROR:请为Nav配置数据源.', '数据源配置出错')
        }
        // data处理
                 ...
                 if(cacheNav.larry  === undefined){
                 	$.ajax({
                 		...
                 		error: function(xhr,status,error){
                 			common.cmsError('"ERROR"'+ error,'Nav缓存开启错误！');
                 		},
                 		complete: function(){
                            _that.config.elem = $container;
                 		}
    ...
    Navbar.prototype.on = function(events,callback){
        ...
        if(typeof(events) !== 'string'){
        	common.cmsError('ERROR:事件名配置出错.','事件名配置出错')
        }
